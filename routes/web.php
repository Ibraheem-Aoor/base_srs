...
use App\Http\Controllers\MoodleAutoLoginController;
...
Route::middleware(['XSS'])->namespace('Web')->group(function () {
    ...
    Route::get('/course/{slug}', 'CourseController@show')->name('course.single');
        // feature
    Route::get('/feature/{slug}', 'FeatureController@show')->name('feature.single');
    // Event Route
...
Route::middleware(['auth:web', 'XSS'])->name('admin.')->namespace('Admin')->prefix('admin')->group(function () {
    ...
    // student auto login moodle
    Route::get('/moodle-login', [MoodleAutoLoginController::class, 'login'])->name('moodle.login');
